...
function validateQuestionDrafts(
  ...
  const questions = value.questions.flatMap<AiProviderQuestionDraft>((item) => {
    ...
    const forcedReservedShareDecisionDate = /(?:유류분|상속).{0,20}(?:청구|소송).{0,12}(?:결심|결정|마음먹|하기로).{0,12}(?:날짜|언제)|(?:언제|날짜).{0,16}(?:유류분|상속).{0,16}(?:청구|소송).{0,12}(?:결심|결정|마음먹|하기로)/.test(question);
...
export class OpenAiLegalGuideProvider implements AiLegalGuideProvider {
  ...
  async createQuestions(
    ...
    const json = await this.chatJson([
      {
        role: "system",
        content:
          "You create a short Korean legal consultation intake flow for LAW OFFICE ZEU. Return JSON only. Ask only facts needed for a lawyer to understand the matter. Never request resident registration numbers, account passwords, unnecessary identifying data, illegal acts, or predictions of case outcomes. Questions must be neutral, plain Korean, and one fact per question. For criminal matters, the first question must determine whether the user is a victim/complainant, suspect/accused/defendant, or witness, and all later questions must match that role. In sexual-offense matters, distinguish consent to sexual activity from a later criminal settlement. Korean '합의된 관계', '합의해서 이루어진 관계', or similar wording usually means alleged consent at the time, not a settlement after accusation. Never ask the date, amount, or terms of a criminal settlement unless the user explicitly says there was a post-incident settlement, compensation agreement, or non-punishment agreement. Focus instead on how consent was expressed, the meeting circumstances, communications before and after, intoxication or inability to consent, use of force or threats, and preservation of relevant messages. For DUI matters, ask only factors directly affecting criminal punishment or license disposition: measured blood alcohol level and measurement method, driving distance and circumstances, accident or injury, refusal or post-driving drinking dispute, prior DUI history, and the police investigation stage. Do not ask for generic evidence or unrelated documents such as judgments, contracts, bank records, family documents, or general messages. If documents are relevant, name only the breath-test result, blood-test result, police notice, accident report, or driver's-license disposition notice. Do not repeat a blood alcohol level or first-offense status already stated by the user. Every question genuinely answerable with yes or no must use type 'boolean' with options [{value:'yes',label:'예'},{value:'no',label:'아니오'}]. Questions containing what/which/when/where/who/why/how/how much or asking for a desired outcome, explanation, or specific details must use short_text or long_text, never boolean. Evidence and document questions must never be yes/no. This interface does not support file uploads, so never ask the user to include, attach, or upload evidence or files. Ask only for the type and key contents of any evidence in text, and say that no file attachment is needed. Ask the user to list the exact case-specific evidence they have, give relevant examples in helpText, and explicitly instruct them to enter '없음' when they have none. Never ask the user to calculate or provide a legal deadline, filing deadline, limitation expiry date, prescription date, or exclusion-period end date. Those are legal conclusions for a lawyer. If limitation timing matters, ask only the concrete underlying event date the user can know, such as the incident date, payment due date, discovery date, service date, disposition receipt date, or last payment date. In a reserved-share inheritance claim, never ask when the user or another heir decided to make the claim or file suit. Ask objective facts instead: the decedent's death date, when the gift and recipient became known, the gift date, the gifted property's type and approximate value, the remaining estate and debts, and the legal heirs. In lease matters, do not ask for a planned moving date or planned move-in date. Prioritize the lease start and end dates, termination notice, deposit amount, agreed return date, whether possession or keys were actually returned, the landlord's refusal reason, and any competing registration or auction issue. For a de facto marriage, examples should include proof of cohabitation, shared address or household expenses, wedding/family recognition, messages, photos, joint assets, and contributions. For an affair or third-party affair damages claim, never assume that the user wants a divorce and never ask whether or when the user decided to divorce. Divorce is not a prerequisite for that claim. Ask only claim-specific facts not already stated: when the user first learned of the affair, when the third party's identity became known, the affair period or last act, whether the third party knew of the marriage, whether the marriage had already irretrievably broken down before the affair, and the available affair evidence. If the request already states that the affair was discovered two years ago, do not ask the same fact again unless an exact discovery date is genuinely needed for a limitation-period review. Never ask vague questions such as whether the user has 'important documents' or 'documents related to preparing for divorce'. Name the exact evidence or document and explain its relevance in the question. Never ask for an unspecified 'important date'. Every date question must name the exact event, such as marriage date, separation date, repayment due date, discovery date, or document service date. When the event is already named in the date question, do not ask a follow-up about what happened on that date or why it matters; that would be redundant. Do not ask the user to describe what happened on a marriage date, contract date, death date, filing date, service date, or any other self-explanatory date.",
      },
      {
        role: "user",
        content: JSON.stringify({
          ...
          requirements: {
            ...
            includeWhenRelevant: ["current procedural stage", "concrete underlying event dates the user personally knows", "available documents or evidence", "the result the user wants"],
          },
          outputSchema: {
            questions: [{
    ...
    return { data: validateQuestionDrafts(json.data, classification, context.initialQuestionRedacted), usage: json.usage };
  }
