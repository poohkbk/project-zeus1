  ...
  if (classification.category === "inheritance") {
    ...
    if (caseType === "estate-division" || /상속재산\s*분할|상속재산분할|상속분|기여분/.test(initialQuestion)) return {
      missingInformation: [
        ...
        "부모님을 부양한 상속인이 있다면 부양한 시작일과 종료일 또는 총 기간을 확인해주세요.",
        "동거·간병·생활비나 치료비 부담 등 구체적인 부양 방식과 비용을 확인해주세요.",
        "그 부양이 다른 자녀의 통상적인 부양과 비교해 특별한 기여였는지 및 다른 상속인도 함께 부양했는지 확인해주세요.",
        "특정 상속인이 생전 증여를 받은 사실이 있는지 확인해주세요.",
